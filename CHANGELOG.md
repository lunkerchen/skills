...
- **README quick cards & routing GIF**: both READMEs gain a Quick Cards section — a bilingual 12-cell card grid (10 flagship + 2 standalone, each cell linking its SKILL.md) plus an ~89 KB two-stage routing demo GIF (prompt → root-router sweep → suite lock → eval badge), regenerable offline via `docs/assets/make_routing_gif.py` (Pillow only).
