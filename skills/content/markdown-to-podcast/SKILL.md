...
#### 聲音複製（Voice Cloning） — 使用者的聲音

使用者的參考音檔位於 `$VOICE_CLONE_AUDIO_DIR/`（DJI 系列 WAV，5-14 秒）。使用 `reference_wav_path` 參數取代 `speaker_centroid`：

```python
from bluemagpie import BlueMagpieModel
audio = model.generate(
    target_text="要朗讀的文字",
    reference_wav_path="$VOICE_CLONE_AUDIO_DIR/DJI_27_20260401_180028.WAV",
    cfg_value=2.8,
...
- **Edge TTS** — 微軟神經網路語音，比 macOS `say` 自然很多
  ...
  - 繁中台灣語音（3 種）:
    ...
    - `zh-TW-YunJheNeural` — 男聲，友善正向（使用者預設試聽中）
...
使用者偏好：**85% 速度**（`atempo=0.85`）— 更從容、不趕。

## 使用者偏好（user）

- **TTS（從優到劣，考量清晰度）**: **BlueMagpie-TTS hung_yi_lee（台灣口音）**（最佳平衡）≈ **Edge TTS**（快速）> **BlueMagpie-TTS 聲音複製（自己的聲音）**（自然但注意清晰度陷阱）> macOS `say`（罐頭感）
- **語音**：YunJheNeural（男聲，Edge TTS 選項）；hung_yi_lee（BlueMagpie 選項）；自己的聲音（voice clone 參考 `$VOICE_CLONE_AUDIO_DIR/`，但注意斷句陷阱）
...
- **輸入來源**：`vocus-drafts/` 下的 `.md` 檔（非原先的 briefings/）
  - 路徑：`$OBSIDIAN_VAULT/04-HERMES-OUTPUTS/vocus-drafts/`
...
- **ffmpeg atempo 品質**：`atempo=0.85`（約慢 15%）是 使用者偏好的語速。注意 atempo 範圍只能 0.5-2.0，且偏離 1.0 太多會產生 artifact。如果需要大幅降速，分兩次串接：`atempo=0.85,atempo=0.85` 而非 `atempo=0.72`。
