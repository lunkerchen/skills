...
2. **Export to notes** → If the user asks to save to notes (e.g. "輸出到筆記"), write to their Obsidian vault. For this user (Engineer), the target is `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/Engineer/05-HERMES-OUTPUTS/`. Maintain the YAML frontmatter with source URL, channel, date, and tag the note appropriately.
