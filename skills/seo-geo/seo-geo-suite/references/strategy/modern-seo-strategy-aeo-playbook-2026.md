...
## 十四、學術批判與防反噬紅線（2026/9 實證更新：破除 AEO 泡沫）

> 參考來源：Stacy Muur 2026/9 覆盤、Olivier Martinez「A Critical Survey on GEO」（arXiv:2607.14035，2026/7）、Lily Ray（2026/3）、Pew Research（2025/7）。

### 1. 商業宣傳 vs 獨立學術的真相差距
- **案例的偏誤與不可重現性**：商業機構宣傳的爆發案例（如 Ramp 可見度 3.2%→22.2%、Novig 27 倍流量），大多由 AEO 工具商或代操機構發布，未做因果變因隔離，且多數關鍵指標（如真實引用率、長期轉換）未同步增長。
- **C-SEO Bench 實測**：在 54 種「戰術 × 領域」測試組合中，**僅 3 種具備統計顯著的正向效應**；在最熱門的「Q&A 問答」類別中，正向效應為 **0**。
- **SAGEO Arena 負效應警告**：刻意針對內文結構進行「迎合 AI 抽取」的過度優化，實測反而使引用頻率**降低 6% 至 9%**。
- **普林斯頓基準重估**：業界普遍引用的「+40% 可見度提升」（arXiv:2311.09735），在獨立複現審查中被證實高度依賴「5 篇預設文檔封閉模擬器」，無法泛化至真實開放 Web。

### 2. 點擊行為真相：1% 點擊率與 4.4 倍轉換率
- **Pew 70K 搜尋實測**：Google 出現 AIO 時，傳統結果點擊率從 15% 跌至 8%（-47% 相對跌幅）；更關鍵的是，**僅 1% 使用者會點擊 AIO 區塊內的來源連結**。
- **價值重新定位**：AEO 無法拯救點擊總量；但 Semrush 實證 AI 轉介訪客轉換率是傳統搜尋的 **4.4 倍**。實務策略應鎖定「高客單、高決策成本、高意圖交易關鍵字」，放棄為大眾資訊詞爭取無效曝光。

### 3. 模型生命週期波動（Reddit 案例）
- **Promptwatch 實測**：Reddit 曾佔 ChatGPT 總引用的 ~3%，但在一次底層模型更新後，其在 ChatGPT 的引用份額於**一週內暴跌 95%**。
- **啟示**：傳統 SEO 演算法週期為 1–2 年，具備較高資產黏性；AEO 戰術高度依附模型內部權重與 prompt routing，極易隨模型換代瞬間失效。不可將業務核心獲客完全押注在單一 AEO 戰術上。

### 4. 激進 GEO 戰術遭遇 Google 演算法懲罰
- **Lily Ray 案例庫**：SEO 團隊若為了衝刺 GEO/AEO 分數，採取「大量 AI 產出問答」、「人造新鮮度（偽造修改時間）」或「頁面嵌入 AI 摘要按鈕」等投機戰術，往往短期衝高後，在接下來的 Google 核心演算法更新中遭遇「全站斷崖式降權」。
- **防禦底線**：回歸真實深度內容、第一手研究證據與站外自然提及（Earned Media / Third-party mentions），切勿因 AEO 犧牲人類讀者體驗。
